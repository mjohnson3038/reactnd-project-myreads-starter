...
class ListShelves extends React.Component {
  ...
  addBookToShelf = (shelf, bookToAdd, author, image) => {
    let booksOnShelves = this.state.books
    const updatedShelf = this.state.books[shelf].concat({title: bookToAdd, author: author, image: image});
  ...
  finishMovingBook = (event) => {
    ...
    if (targetShelf !== 'none') {
      this.addBookToShelf(targetShelf, bookToMove, author, image)
    }
